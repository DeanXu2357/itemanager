...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun insertAll(items: List<Item>) = viewModelScope.launch(Dispatchers.IO) { itemDao.insertAll(items) }

    fun deleteAll(items: List<Item>) = viewModelScope.launch(Dispatchers.IO) { itemDao.deleteAll(items) }

    fun getItemById(id: Long) = itemDao.getItemById(id)

    fun getItemByBarcode(barcode: String) = viewModelScope.launch(Dispatchers.IO) { itemDao.getItemByBarcode(barcode) }
    ...
    fun setSorting(sortBy: String, sortOrder: String) {
        ...
        // Note: In a real app, you might want to use MutableStateFlow for currentSortBy and currentSortOrder
        // and combine them with the Pager to automatically refresh when they change
    }

    // ItemType operations
    fun insertItemType(itemType: ItemType) = viewModelScope.launch(Dispatchers.IO) { itemTypeDao.insert(itemType) }

    fun updateItemType(itemType: ItemType) = viewModelScope.launch(Dispatchers.IO) { itemTypeDao.update(itemType) }

    fun deleteItemType(itemType: ItemType) = viewModelScope.launch(Dispatchers.IO) { itemTypeDao.delete(itemType) }
    ...
    fun itemTypeExists(name: String) = viewModelScope.launch(Dispatchers.IO) { itemTypeDao.itemTypeExists(name) }
